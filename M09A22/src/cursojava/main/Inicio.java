...
public class Inicio {
	...
	public static void main(String[] args) {
		...
		if (acao1 == 0) {
			...
			int acao2 = JOptionPane.showConfirmDialog(null, "Deseja remover um Aluno no sistema?");

			if (acao2 == 0) {
				String nomeAlunoProcurado = JOptionPane.showInputDialog("Qual é o nome do aluno que deseja remover?");
				for (Aluno aluno : alunos) {
					if (aluno.getNome().equalsIgnoreCase(nomeAlunoProcurado)) {

						alunos.remove(aluno); // removendo aluno
						System.out.println("Aluno, " + nomeAlunoProcurado + " foi excluido da lista");
				...
				// caso a acao for nao ==1 , ou cancela ==2 imprime o relatorio todo
				for (Aluno aluno : alunos) {
